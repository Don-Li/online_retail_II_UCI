...
Dear reader, this document is available in `.pdf` and `.html` formats. There is a lot of code and some of it is quite long (e.g., the model that I coded up). The `.pdf` format does not allow code folding, so I've moved some of the code around and/or chosen only to show specific code chunks. Aside from that the `.pdf` and `.html` versions have the same content.
...
A collection of exploratory plots. I made an aggregator function (Appendix 1, `time_aggregator`). There are other ways to aggregate, such as making a `ts` object and using `aggregate`. But, I prefer the lower-level interface.
...
```{r fig.cap="Figure 1. Daily sales volume.", class.source='fold-hide', echo=FALSE}
aggregate_scale = "Day"
...
```{r fig.cap="Figure 2. Weekly sales volume.", class.source='fold-hide', echo = FALSE}
...
```{r fig.cap="Figure 3. Monthly sales volume.", class.source='fold-hide', echo = FALSE}
...
```{r fig.cap="Figure 4. Revenue share by product (stock code)", class.source='fold-hide', echo = FALSE}
revenue_by_product[ , {
...
```{r fig.cap="Figure 5. Revenue share by customer", class.source='fold-hide', echo=FALSE}
revenue_by_customer[ , {
...
```{r fig.cap="Figure 6. Weighted average monthly sale by volume", class.source='fold-hide', echo=FALSE}
...
2. The returns are a data entry error. There are some refunds that occurred over a year after the data collection start that could not be matched to purchases. In my view, it is unlikely for customers to be claiming refunds on goods purchased over a year ago. Example below:
...
# We see that customer 17337 has made two refunds of 12 units of item 22087.
```


3. The third option for negative sales quantities is that the invoicing system is used for inventory write-offs (e.g.,  defective inventory), samples for clients, bank charges, and postage. Example below: 
```{r class.source='fold-hide', echo = FALSE}
...
Code is in Appendix `model data cleaning`
...
For completeness, I will also removed the matched refunds but throughout the dataset. The code is in Appendix `model data cleaning refunds`.
...
The code is quite long, so it is included in Appendix `model data frame`
...
```{r fig.cap="Figure 8. Differenced daily sales volume.", class.source='fold-hide', eval = FALSE, echo = FALSE}
model_data[ , {
...
The `JAGS` code for the model is included in Appendix `Bayesian model`.

```{r class.source='fold-hide', eval = FALSE, echo = FALSE}
{jags_model = "
    model{
    # Priors
...
```{r fig.cap="Figure 8. Historical daily sales volume.", class.source='fold-hide', echo=FALSE}
...
```{r fig.cap="Figure 9. Historical daily gross sales.", class.source='fold-hide', echo=FALSE}
model_data[ , {
...
```{r fig.cap="Figure 10. Predicted daily gross sales for December 2011.", class.source='fold-hide', echo=FALSE}
...
```{r fig.cap="Figure 11. Predicted December 2011 revenue.", class.source='fold-hide', echo = FALSE}
...
```{r fig.cap="Figure 11. Gross sales for December 2010.", class.source='fold-hide', echo=FALSE}
...
A function for aggregating results from any computed function across day, week, month, year timescales.
...
# Make the data frame for the data up to November and also the 
# Partial data so that we can make predictions
model_data_dec_only_JAGS = copy( model_data_dec_only )
model_data_dec_only_JAGS = model_data_dec_only_JAGS[ , {
    list(sales_volume, InvoiceMonth, InvoiceWD, InvoiceDay,
            InvoiceYear, gross_sales)
