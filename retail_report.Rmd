...
Figures 8 and 9 show the historical daily sales volume and gross sales. 95% credible intervals for the posterior predicted volume/sales are also shown. The predictions are shown up to November 2012. I will zoom in on the December 2012 predictions subsequently. Due to the high variability of the time-series, the credible intervals are very wide. However, our model does seem to capture the overall trends on the month and weekly scales.
...
model_data[ , {
    plot( period_end, sales_volume/ 1000, type = "l", ylab = "Sales volume / 1000",
        xlab = "Day", main = "Daily sales volume (up to end Nov 2012)")
...
model_data[ , {
    plot( period_end, gross_sales/1000, type = "l", ylab = "Gross sales / 1000",
        xlab = "Day", main = "Daily gross sales (up to end Nov 2012)")
...
posterior_predict_dec_2012 = rowSums( dec_gross_sales_pred ) +
    sum(model_data_dec_only$gross_sales, na.rm = T )
posterior_predict_dec_2012_CI = round(
    quantile(posterior_predict_dec_2012, probs = c(0.05, 0.95)), -4 )

```

The predicted gross sales for December 2012 are shown in Figure 10. The daily volume graph is omitted for brevity. The break in the series between the data and the predicted values is a Saturday, which was removed from the dataset. Figure 11 shows the distribution of the predicted revenue for December 2012. Based on our model, the predicted revenue for December is expected to be $`r round( mean(posterior_predict_dec_2012), -4 )`, and between `r posterior_predict_dec_2012_CI[1]` and `r posterior_predict_dec_2012_CI[2]` with 90% probability. 

```{r fig.cap="Figure 10. Predicted daily gross sales for December 2012.", class.source='fold-hide'}
...
model_data_dec_only[ , {
    plot( period_end, gross_sales/1000, type = "l", ylab = "Gross sales / 1000",
        xlab = "Day", main = "Daily gross sales (up to end Nov 2012)", ylim = c(0, 100))
...
```{r fig.cap="Figure 11. Predicted December 2012 revenue.", class.source='fold-hide'}
hist( posterior_predict_dec_2012,
    main = "Predicted revenue December 2012", breaks = 100,
    ylab = "Density", xlab = "Revenue", prob = T)
invisible(NULL)
```

To improve our confidence in these results, we can examine the predictions for the previous December (2011). From Figure 12, the credible intervals enclose the observed daily gross sales. 

```{r fig.cap="Figure 12. Gross sales for December 2011.", class.source='fold-hide'}
...
model_data_no_dec[ period_end %in% prediction_dates, {
    plot( period_end, gross_sales/1000, type = "l", ylab = "Gross sales / 1000",
        xlab = "Day", main = "Daily gross sales (Dec 2011)", ylim = c(0, 100))
...
The average predicted revenue for December 2011 was \$`r pred_december_revenue_mean`, with a 90% credible interval of `r pred_december_interval[1]` to `r pred_december_interval[2]`. This is compared to an actual revenue for December 2011 of \$`r december_revenue`. So, based on the model's historical performance, we except our forecast for December 2012 to be fairly accurate.

## Conclusion

For December 2012, I forecast an expected revenue of $`r round( mean(posterior_predict_dec_2012), -4 )`, with 90% probability being between `r posterior_predict_dec_2012_CI[1]` and `r posterior_predict_dec_2012_CI[2]`. Based on forecast accuracy for last year (December 2011), I am quite confident in the accuracy of my predictions for 2012. Of course, this is assuming that past trends continue to hold into the future. Overall, I can back my prediction enough to recommend a Ferrari.
